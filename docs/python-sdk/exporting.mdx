...
First install the OpenTelemetry application in your Observe workspace. From that OpenTelemetry application, create a new  OpenTelemetry token. You will also need your customer id to construct the reporting URL (the prefix of all your Observe URLs):

```bash
TRACELOOP_BASE_URL=https://{customer id}.collect.observeinc.com/v1/otel
TRACELOOP_HEADERS="authorization=Bearer%20{token}"
```
